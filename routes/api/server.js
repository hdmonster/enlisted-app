...
router.post('/create', isAdmin ,async(req, res, next) => {
    ...
    try {
        ...
        const userServer = await db.doc(`users/${userId}`).update('servers',[createServer.id]);
...
router.post('/join', async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            const addToUser = await db.doc(`users/${userId}`).update('servers',[server_code]);
            req.flash('You have been added to the server');
            res.redirect('back');
        } catch (error) {
